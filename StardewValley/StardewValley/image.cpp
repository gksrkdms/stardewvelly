...
void image::floatRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, float scalar)
{
	if (m_isTransparent)
	{
		GdiTransparentBlt(
...
void image::frameRenderTile(HDC hdc, int destX, int destY, int currFrameX, int currFrameY, int tilesizeX, int tilesizeY)
{
	...
	if (m_isTransparent)
	{
		GdiTransparentBlt(
			hdc,	// 복사될 목적지 DC
			destX, destY,		// 복사될 좌표 시작점
			m_pImageInfo->nFrameWidth*tilesizeX,
			m_pImageInfo->nFrameHeight*tilesizeY,	// 복사될 크기
...
void image::frameRenderMini(HDC hdc, int destX, int destY, int currFrameX, int currFrameY, int scalar,int tilesizeX, int tilesizeY)
{
	m_pImageInfo->nCurrFrameX = currFrameX;
	m_pImageInfo->nCurrFrameY = currFrameY;
...
void image::rotateRender(HDC hdc, float rotateAngle, float fX, float fY, int scalar/*=1*/)
{
	...
	if (m_isTransparent)
	{
		...
		rc = RectMake(0, 0, m_pImageInfo->nWidth, m_pImageInfo->nHeight);
	...
	else
	{
		...
		PlgBlt(hdc, m_vertices, m_pImageInfo->hMemDC, 0, 0, m_pImageInfo->nWidth, m_pImageInfo->nHeight, NULL, 0, 0);
...
void image::alphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, BYTE alpha, int scalar)
{
	m_blendFunc.SourceConstantAlpha = alpha;
...
void image::alphaFrameRender(HDC hdc, int destX, int destY, int currFrameX, int currFrameY, BYTE alpha, int scalar)
{
	m_pImageInfo->nCurrFrameX = currFrameX;
	m_pImageInfo->nCurrFrameY = currFrameY;
...
void image::floatAniRender(HDC hdc, int destX, int destY, animation * ani, float scalar)
{
	floatRender(hdc, destX, destY,
		ani->getFramePos().x, ani->getFramePos().y,
		ani->getFrameWidth(), ani->getFrameHeight(), scalar);
}
